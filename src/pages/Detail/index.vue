          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(saleAttr, index) in spuSaleAttrList" :key="saleAttr.id">
                ...
                <dd changepirce="0" :class="{ active: saleAttrValue.isChecked == 1 }" v-for="(saleAttrValue, index) in saleAttr.spuSaleAttrValueList" :key="saleAttrValue.id" @click="changeChecked(saleAttrValue, saleAttr.spuSaleAttrValueList)">
...
export default {
  ...
  methods: {
    ...
    //加入购物车按钮的回调函数
    async addShopCart() {
      //以前我们经常点击按钮进行路由跳转，但是需要注意这里，点击添加购物车按钮的时候，需要发请求的
  ...
  //组件内守卫之一：一般用于带有动态参数路由【params参数：当params参数发生变化的时候会触发】
